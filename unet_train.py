...
filepath ='D:/Deep Learning/SegNet/dataset/train/'

def get_train_val(val_rate = 0.25):
    train_url = []    
    train_set = []
    val_set  = []
    for pic in os.listdir('D:/Deep Learning/SegNet/dataset/train/src'):
...
def generateData(batch_size,data=[]):  
    ...
    while True:  
        ...
        for i in (range(len(data))): 
            url = data[i]
            batch += 1 
            img = load_img('D:/Deep Learning/SegNet/dataset/train/src/' + url)
            img = img_to_array(img)  
            train_data.append(img)  
            label = load_img('D:/Deep Learning/SegNet/dataset/train/label/' + url, grayscale=True)
